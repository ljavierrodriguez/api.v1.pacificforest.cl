...
# auto_error=False permite fallback a cookie cuando no viene header Authorization
...
def _extract_token_from_cookie(request: Request) -> Optional[str]:
    ...
    if raw_cookie.lower().startswith("bearer "):
        return raw_cookie[7:].strip() or None
...
def _extract_token_from_query(request: Request) -> Optional[str]:
    ...
    raw_query_token = raw_query_token.strip()
    if raw_query_token.lower().startswith("bearer "):
        return raw_query_token[7:].strip() or None

    return raw_query_token
...
def get_current_user(
    ...
    resolved_token = token or _extract_token_from_cookie(request) or _extract_token_from_query(request)
