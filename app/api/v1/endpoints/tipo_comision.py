...
@router.get("/", response_model=List[TipoComisionRead], summary='GET TipoComision', description='Obtener lista de tipos de comisión con paginación.')
def list_tipo_comision(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return db.query(TipoComision).offset(skip).limit(limit).all()
